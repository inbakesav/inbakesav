<img width="1584" height="396" alt="B Tech - Artificial Intelligence  and Data Science" src="https://github.com/user-attachments/assets/2d018701-654a-4765-b15c-bb85d0940b49" />

# Hi, I'm Inbarasi 👋

🎓 B.Tech Artificial Intelligence & Data Science student  
💡 Passionate about Data Analytics, Machine Learning, and building data-driven solutions  

---

## 🔹 About Me
I am an AI & Data Science student with strong foundations in Python, SQL, and data analytics.  
I enjoy working on real-world projects involving data preprocessing, visualization, and predictive modeling.

---

## 🔹 Skills
- Python
- SQL
- Pandas
- Matplotlib, Seaborn
- Power BI
- Machine Learning (Basics)

---

## 🔹 Projects

### 📊 Student Performance Prediction
- Built a machine learning model using student academic data  
- Performed data cleaning, preprocessing, and EDA  
- Applied ML algorithms and evaluated performance  

### 📈 Sales Dashboard (Power BI)
- Designed an interactive dashboard to analyze sales performance  
- Visualized revenue trends and key business insights  

---

## 🔹 Currently Learning
- Machine Learning
- Data Science

---

## 🎯 Goal
To build impactful data-driven solutions and grow as a Data Scientist.

---

## 🔗 Connect with Me
- LinkedIn: https://www.linkedin.com/in/inbarasi-m-401b74333  
- Email: inbakesav@gmail.com
